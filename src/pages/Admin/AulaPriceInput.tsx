import { Button } from '@material-tailwind/react';
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ADD_TIPE_KAMAR, GET_AULA_PRICE } from '../../api/routes';
import { API_STATES, MODAL_TYPE } from '../../common/Constants';
import Breadcrumb from '../../components/Breadcrumbs/Breadcrumb';
import { useModal } from '../../components/Provider/ModalProvider';
import useFetch from '../../hooks/useFetch';
import { formatCurrency, parseCurrency } from '../../utils/Utility';

const AulaPriceInput = () => {
  const [harga, setHarga] = useState<string>('');

  // modal
  const { toggle, setType, setOnConfirm } = useModal();

  // button
  const isButtonDisabled = !harga;

  useEffect(() => {
    getAulaPrice();
  }, []);

  async function getAulaPrice() {
    setType(MODAL_TYPE.LOADING);

    const { state, data, error } = await useFetch({
      url: GET_AULA_PRICE,
      method: 'GET',
    });

    if (state == API_STATES.OK) {
      setHarga(formatCurrency(data.aula_price));
    }
  }

  async function onUpdatePrice() {
    setType(MODAL_TYPE.LOADING);

    const { state, data, error } = await useFetch({
      url: GET_AULA_PRICE,
      method: 'POST',
      data: {
        price: parseCurrency(harga),
      },
    });

    if (state == API_STATES.OK) {
      setType(MODAL_TYPE.SUCCESS);
      setOnConfirm(() => {
        getAulaPrice();
        toggle();
      });
    } else {
      setType(MODAL_TYPE.ERROR);
      setOnConfirm(() => toggle());
    }
  }

  const handleHargaChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputValue = e.target.value;
    setHarga(formatCurrency(inputValue));
  };

  return (
    <>
      <Breadcrumb pageName="Edit Harga Aula" />

      <div className="w-full">
        <div className="flex flex-col gap-9">
          {/* <!-- Contact Form --> */}
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark">
              <h3 className="font-medium text-black dark:text-white">
                Form Aula
              </h3>
            </div>
            <form action="#">
              <div className="p-6.5">
                <div className="mb-4.5">
                  <label className="mb-2.5 block text-black dark:text-white">
                    Harga Sewa Aula / Hari
                  </label>
                  <input
                    type="text"
                    placeholder="Masukan harga aula"
                    value={harga}
                    onChange={handleHargaChange}
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  />
                </div>
                <Button
                  disabled={isButtonDisabled}
                  color={'blue'}
                  fullWidth
                  className=" normal-case mt-8"
                  onClick={() => {
                    setType(MODAL_TYPE.CONFIRMATION);
                    setOnConfirm(() => onUpdatePrice());
                    toggle();
                  }}
                >
                  Simpan
                </Button>
              </div>
            </form>
          </div>
        </div>
      </div>
    </>
  );
};

export default AulaPriceInput;
